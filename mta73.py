...
an2 = datetime.datetime(2099,8,8)
hours = (now.hour)
if an > an2 or an == an2:
...
def info(username,jj):
  ...
  try:
    ...
    try:
      ...
      date='None'
    tlg = f'''
𝐅𝐨𝐥𝐥𝐨𝐰𝐞𝐫𝐬 : {user_followers}
𝐅𝐨𝐥𝐥𝐨𝐰𝐢𝐧𝐠 : {user_following}
𝐓𝐨𝐭𝐚𝐥 𝐏𝐨𝐬𝐭𝐬 : {total_posts}
𝐔𝐬𝐞𝐫𝐧𝐚𝐦𝐞 : {username}
𝐄𝐦𝐚𝐢𝐥 : {username}@{jj}
𝐃𝐚𝐭𝐞 : {date}
𝐈𝐃 : {id}
...
def check(email):
  ...
  try:
    ...
    tt=(f"\r{GREEN}Hits:{GREEN} {hits} {RED}Bad insta:{RED} {bads_instgram} {YELLOW} Not:{YELLOW} {bads_email}")
    print(tt)
    if pp == '0':
      headers = {
        'accept': '*/*',
